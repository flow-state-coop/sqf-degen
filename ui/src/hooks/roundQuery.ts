...
import useStreamingQuadraticFunding from "./streamingQuadraticFunding";
...
import { DEGENX_ADDRESS, GDA_CONTRACT_ADDRESS } from "../lib/constants";
...
export default function useRoundQuery(userAddress?: Address) {
  ...
  const { recipients, gdaPool } = useStreamingQuadraticFunding();
  ...
  const { data: userInfoQueryResult, loading: userInfoLoading } = useQuery(
    USER_INFO_QUERY,
    {
      variables: {
        ...
        token: DEGENX_ADDRESS.toLowerCase(),
      },
      pollInterval: 10000,
    }
  );
